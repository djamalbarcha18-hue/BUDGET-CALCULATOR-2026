...
2. **كتلة الدخل** (الصفوف `9:29`): ترويسة ثمانية أعمدة بترتيب RTL من العمود `A` (`التاريخ، الفئة، الوصف، الدخل المتوقع، الدخل الفعلي، المداخيل، الفرق، طريقة الدفع`)، 19 صفّ إدخال، صفّ إجماليات. عمود `المداخيل` أُضيف في الإصدار 1.1 لتسجيل الإيرادات الإضافيّة لكل صفّ، ويُضمّ تلقائيّاً إلى صيغة الفرق وإلى إجمالي الدخل الفعلي في لوحة المؤشّرات.
